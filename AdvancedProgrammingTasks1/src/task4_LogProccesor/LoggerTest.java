...
class LogSystem<T extends ILog & Comparable<T>> {
    ...
    void printResults() {
        ...
        LogProcessor <T>secondLogProcessor =(logs)->
                logs.stream()
                        .filter(i->i.getMessage().length()<100)
                        .collect(Collectors.toCollection(ArrayList::new));
        LogProcessor <T>thirdLogProcessor =(logs)->
                logs.stream()
                        .sorted()
                        .collect(Collectors.toCollection(ArrayList::new));
...
public class LoggerTest {
    public static void main(String[] args) {
        ...
        LogSystem<RealLog> realLogSystem = new LogSystem<RealLog>(realLogs);
        LogSystem<DummyLog> dummyLogSystem = new LogSystem<DummyLog>(dummyLogs);
