package task5_GenericCollection;
...
class GenericCollection<T extends Comparable<T>& IHasTimestamp>{
    //key=category value set od generici
    ...
    public void addGenericItem(String category, T integerElement) {
        map.computeIfAbsent(category,x->new TreeSet<>());
        map.get(category).add(integerElement);
    }
    ...
    public Collection<T> itemsFromCategories (List<String> categories) {
        return map.keySet().stream().
                filter(i->categories.contains(i))
    ...
    public Map<Integer, Long> countByYear() {
        Map<Integer,Long> novaMapa=new TreeMap<>();
        map.values().stream()
                .flatMap(i->i.stream())
                .mapToInt(i->i.getTimestamp().getYear())
                .forEach(i->{
                    novaMapa.putIfAbsent(i, 0L);
                });
        novaMapa.replaceAll((year, count) ->
                count + map.values().stream()
                        .flatMap(Collection::stream)
                        .filter(i -> i.getTimestamp().getYear() == year)
                        .count());
        return novaMapa;
    }
}
